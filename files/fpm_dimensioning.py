...
@argh.arg("--slice", help="Slice of RAM allocated to PHP-FPM. 0 means no RAM, 1 means all the RAM available. Defaults to 0.4.", default=0.4)
...
def write_max_children_config(**kwargs):
    ...
    LOGGER.info("Memory slice ratio to be used by PHP-FPM is %s (%s%% of the total memory available.)", slice, '%d' % (slice * 100))
    LOGGER.info("Calculating max_children for a %sM (in average) memory footprint per thread...", avgmem)

    max_children = 5
    
    if avgmem > 0:
        max_children = get_php_fpm_memory(slice) / avgmem
        if max_children < 5:
